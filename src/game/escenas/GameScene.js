  ...
  create() {
    ...
    if (this.esMultijugador) {
      ...
      multiplayerManager.onPosicionActualizada = (x, y, animacion, sprite, escena) => {
        if (escena !== 'GameScene') {
          if (this.jugadorRemoto) {
            this.jugadorRemoto.destruir();
            this.jugadorRemoto = null;
          }
          return;
        }
        if (!this.jugadorRemoto) this.jugadorRemoto = new JugadorRemoto(this, x, y, sprite);
      ...
      multiplayerManager.onJugadorDesconectado = () => {
        ...
        this.add
          .text(640, 50, 'El otro jugador se desconectó', {
            fontFamily: '"Jersey 10"',
            fontSize: '22px',
            fill: '#ff4444',
